...
/** Build the common <head> + header block used in all print windows */
  ...
  return `
    <style>
      ...
      *{box-sizing:border-box;margin:0;padding:0}
      ...
      th{background:${c.headerBgColor};color:${c.headerTextColor};padding:8px 12px;text-align:left;font-size:${c.fontSize - 2}px;white-space:nowrap}
      .p-header{text-align:center;background:${c.headerBgColor};color:${c.headerTextColor};padding:14px 16px;margin-bottom:18px;border-radius:4px}
      ...
      .p-stamp{border:2px solid ${c.headerBgColor};color:${c.headerBgColor};padding:3px 14px;border-radius:4px;font-weight:bold;font-size:${c.fontSize - 1}px;letter-spacing:1px}
    </style>
    <div class="p-header">
      ${logoHtml}
...
function buildFooter(c: ReturnType<typeof cfg>): string {
  return `
    <div class="p-footer">
      ...
      <span class="p-stamp">${c.orgName}</span>
    </div>
  `;
}
